      ...
      return(
        <div>
            ...
            <div className=' flex flex-col'>
                ...
                {!control ? (<p className='text-white font-bold'>پیغامی وجود ندارد</p>) : (
                    <div>
                        {
                        receiverChats &&
                            receiverChats.map((chat)=> {
                                ...
                                return (
                                    <div className='w-1/2 h-fit rounded-lg bg-green-500 my-5' key={chat.date}>
                        ...
                        { senderChats && 
                            senderChats.map((chat)=>{
                                ...
                                const minute = dateObj.getMinutes();
                                return(
                                <div className='w-1/2 h-fit rounded-lg bg-red-500 my-5 ' key={chat.date}>
                                    <p className='w-full  text-white p-2 '>{chat.text}</p>
                                    <p>{hour}:{minute}</p>
                                </div>);
                             })
...
{/* { 
                ...
                receiverChats &&
                    receiverChats.map((chat)=>(
                        <div className='w-1/2 h-fit rounded-lg bg-green-500 my-5 ' key={chat.id}>
                            <p className='w-full  text-white p-2 '>{chat.text}</p>
                            <p>{chat.date}</p>
                        </div>
                    ))
                ...
                { senderChats && 
                    senderChats.map((chat)=>(
                        <div className='w-1/2 h-fit rounded-lg bg-red-500 my-5 ' key={chat.id}>
                            <p className='w-full  text-white p-2 '>{chat.text}</p>
                            <p>{chat.date}</p>
                        </div>
                    ))
